...
def conjecture_alltheta():
    logs = ['result-tmp/1.8.2023/conjecture.4sensor']
    data = Logger.read_log(logs)
    filename = 'result-tmp/1.8.2023/conjecture.4sensor.png'

    table = defaultdict(list)
    for myinput, output_by_methods in data:
        for method, output in output_by_methods.items():
            table[method].append({myinput.unitary_theta: output.success})
    Y = []
    X = [i for i in range(1, 91)]
    for method, mylist in table.items():
        y = defaultdict(list)
        for key_val in mylist:
    ...
    fig.subplots_adjust(left=0.15, right=0.97, top=0.88, bottom=0.15)
    ax.plot(X, Y, label='Conjecture 1')
    ax.legend()
    ax.set_xlabel('Theta', labelpad=30)
    ax.set_ylabel('Probability of Success', labelpad=30)
    ax.set_title('Conjecture 1 for All Theta')
    fig.savefig(filename)
